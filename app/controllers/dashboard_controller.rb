class DashboardController < ApplicationController
  ...
  def whatsup
    if current_user.god == true
      ...
      @hourly_permits = HourlyPermit.where(start_time: dstart..dend)
      ...
      @permits = Permit.where(user_id: User.where(province: current_user.province, region: current_user.region))
      dstart = Date.today.to_date.beginning_of_day
      dend = Date.today.to_date.end_of_day
      ...
      @hourly_permits = HourlyPermit.where(user_id: User.where(province: current_user.province, region: current_user.region))
      @hourly_permits = @hourly_permits.where(start_time: dstart..dend)
    end
  end
